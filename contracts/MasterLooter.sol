...
// MasterLooter is the master looter of whatever loot the GovernanceToken represents.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GovernanceToken is sufficiently
// distributed and the community can show to govern itself.
//
contract MasterLooter is Ownable, Authorizable, ReentrancyGuard {
    ...
    modifier nonDuplicated(IERC20 _lpToken) {
        require(poolExistence[_lpToken] == false, "MasterLooter::nonDuplicated: duplicated");
        _;
    }
    ...
    function add(
        ...
        require(
            ...
            "MasterLooter::add: lp is already in pool"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Deposit LP tokens to MasterLooter for GovernanceToken allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _ref
        ...
        require(
            _amount > 0,
            "MasterLooter::deposit: amount must be greater than 0"
        );
    ...
    // Withdraw LP tokens from MasterLooter.
    function withdraw(
        uint256 _pid,
        uint256 _amount,
        address _ref
        ...
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "MasterLooter::withdraw: not good");
        if (_ref != address(0)) {
            refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
            refer.globalRefAmount = refer.globalRefAmount - _amount;
        }
    ...
    function safeGovTokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "MasterLooter::safeGovTokenTransfer: transfer failed");
    }
